...
{
    public static class StabilityManager
    {
        public static readonly Dictionary<string, float> LastStaggerTimes = new Dictionary<string, float>();

        [HarmonyPatch(typeof(Character), "SlowDown")]
        public class Character_SlowDown
        {
            [HarmonyPrefix]
            public static void Prefix(ref float _slowVal, ref float _timeTo, ref float _timeStay, ref float _timeFrom)
            {
                var num = CombatTweaksMod.HitStop_Modifier.Value;

                // apply the custom modifier MORE to the "slowVal" value, seems to get better results. Clamp above 0.1 otherwise the attack can last like 30+ seconds
                _slowVal = Mathf.Clamp(num * num, 0.1f, 2) * _slowVal;
                // apply MUCH LESS to "timeTo", for same reason.
                _timeTo = ((4 + num) / 5) * _timeTo;
                // apply raw modifier to these two
                _timeStay = num * _timeStay; 
        ...
        //                {
        //                    float hitToStagger = m_shieldStability + m_stability - (100 - (float)CombatTweaksMod.config.GetValue(Settings.Stagger_Threshold));
        //                    if (hit < hitToStagger)
        //                    {
        //                        hit = hitToStagger;
        //                    }
        ...
        //                if (m_stability <= (float)CombatTweaksMod.config.GetValue(Settings.Knockdown_Threshold)
        //                    || (self.IsAI && m_knockbackCount >= (float)CombatTweaksMod.config.GetValue(Settings.Enemy_AutoKD_Count)))
        //                {
        ...
        //                else if (m_stability <= (float)CombatTweaksMod.config.GetValue(Settings.Stagger_Threshold) && (Time.time - lastStagger > (float)CombatTweaksMod.config.GetValue(Settings.Stagger_Immunity_Period)))
        //                {
        ...
        //                    if (m_stability <= (float)CombatTweaksMod.config.GetValue(Settings.Stagger_Immunity_Period))
        //                    {
        ...
        //    if ((bool)CombatTweaksMod.config.GetValue(Settings.Blocking_Staggers_Attacker))
        //    {
        ...
        //            if ((bool)CombatTweaksMod.config.GetValue(Settings.No_Stability_Regen_When_Blocking) && self.Blocking) // no stability regen while blocking! otherwise too op
        //                return false;

        //            float num = Time.time - m_timeOfLastStabilityHit;
        //            if (num > (float)CombatTweaksMod.config.GetValue(Settings.Stability_Regen_Delay))
        //            {
        //                if (m_stability < 100f)
        //                {
        //                    var num2 = Mathf.Clamp(m_stability + (self.StabilityRegen * (float)CombatTweaksMod.config.GetValue(Settings.Stability_Regen_Speed)) * Time.deltaTime, 0f, 100f);
        //                    At.SetField(self, "m_stability", num2);
        ...
        //                }
        //                if (num > (float)CombatTweaksMod.config.GetValue(Settings.Enemy_AutoKD_Reset_Time))
        //                {
        ...
        //        if (At.GetField(self, "m_stability") is float m_stability)
        //        {
        //            float staggerVal = Mathf.Clamp(m_stability - (float)CombatTweaksMod.config.GetValue(Settings.Stagger_Threshold), 
        //                                            1f,
        //                                            100 - (float)CombatTweaksMod.config.GetValue(Settings.Stagger_Threshold));
